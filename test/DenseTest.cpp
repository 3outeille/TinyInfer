...
int main(int argc, const char* argv[]) {
  ...
  auto sampleOp_node = std::make_shared<DenseOp>(input_node);
  Eigen::Tensor<TENSOR_DATA_TYPE, 2, Eigen::RowMajor> c_2d(2, 2);
  c_2d(0, 0) = 6;
  c_2d(0, 1) = 4;
  c_2d(1, 0) = 2;
  c_2d(1, 1) = 0;
  sampleOp_node->register_weight(std::make_shared<runtime::Tensor>(c_2d));

  Eigen::Tensor<TENSOR_DATA_TYPE, 1, Eigen::RowMajor> b_1d(2);
  b_1d(0) = 3;
  b_1d(1) = 1;
  sampleOp_node->register_bias(std::make_shared<runtime::Tensor>(b_1d));
