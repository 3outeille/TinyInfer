...
//    }
    ...
    std::cout << "time collapsed: " << std::chrono::duration_cast<std::chrono::seconds>(finish - start).count() / 10.0
              << " s\n";
    ...
    for (int i = 0; i < output_eigen.dimension(0); i++) {
        for (int j = 0; j < output_eigen.dimension(1); j++) {
            if (std::abs(output_eigen(i, j) - expect_output_eigen(i, j)) > 1e-6) {
                std::cerr << "[WARN] " << output_eigen(i, j) << " does not match with expected "
                          << expect_output_eigen(i, j) << std::endl;
            }
        }
    }
